...
def generate_random_string(len=5):
    return get_random_string(len, ascii_lowercase + digits)

def create_user_if_not_exists_from_bankplusone(bankplusone_user):
    uuid = str(bankplusone_user["id"])
    user_q = User.objects.filter(uuid=uuid)
    if user_q.exists():
        return user_q.first()
    data = {
            "uuid": uuid,
            "company_id": bankplusone_user["company"],
